...
# -*- coding: utf-8 -*-
"""
WellCorrelator v6.5 — 顶部工具栏 + UI美化 + 多道间距优化版
- 美化左侧面板深色主题 UI
- 树形折叠按钮可见（文字箭头 ▾/▸）
- 多道 X 轴间距归零，单道宽度可拖拽调整
- 新增可折叠顶部工具栏，集成：参考井A深度/曲线范围/充填/Ghost/批量分层
  以及目标井B曲线范围/充填/Ghost/批量分层，保存/加载分层快捷按钮
- 不破坏原有数据模型、保存格式与交互逻辑
"""
...
QTreeWidget::branch:closed:has-children:has-siblings {
    ...
    image: none;
    background: #1A2235;
    /* Unicode right-pointing triangle rendered via color trick */
    color: #64748B;
}
...
/* Use primitive arrows for branch indicators */
...
class WellPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # ── 多道布局：GraphicsLayoutWidget ──
        self.gw = pg.GraphicsLayoutWidget()
        self.gw.ci.layout.setSpacing(0)      # 多道间距归零
        ...
        main_pi.addItem(self.hover_label)
    ...
    def add_track(self, curve_name: str) -> bool:
        """从井数据中添加一条新道。返回 True 表示成功。"""
        ...
        pi.setMaximumWidth(16777215)   # allow free resize

        pi.setYLink(self.primary_track.plot_item)   # 共享 Y 轴
        pi.hideAxis('left')                          # 共享 Y 轴，不重复显示
        pi.showGrid(x=False, y=False, alpha=0.3)
        pi.invertY(True)

        # 让每道列宽可独立拉伸（等权重，用户可拖分隔线）
    ...
    def remove_track(self, curve_name: str):
        ...
        for i, t in enumerate(self._tracks):
            if t.curve_name == curve_name and not t.is_primary:
                # 从 plot_item 移除所有 item
                t.plot_item.clear()
                # 从 GraphicsLayout 移除该列
    ...
    def _draw_zones(self):
        """兼容旧调用：在主道上画 Zone"""
        self._draw_zones_on(self.primary_track.plot_item)
    ...
    def _draw_tops(self):
        """兼容旧调用：在主道上画 Top"""
        self._draw_tops_on(self.primary_track.plot_item)
    ...
    def _rebuild_layout(self):
        ...
        self._tracks[0].plot_item.addItem(self.hover_label)
...
class WellTreePanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._slot_well_items = {}   # slot -> QTreeWidgetItem (well node)
        self._slot_log_items  = {}   # slot -> QTreeWidgetItem (Logs folder)
        self._slot_top_items  = {}   # slot -> QTreeWidgetItem (Topset folder)
        self._well_names      = {}   # slot -> str
    ...
    def _build_ui(self):
        ...
        t2 = QLabel("v6.5  ·  StarSteer Style")
        ...
        self.tree.setDragDropMode(QAbstractItemView.DragDropMode.DragOnly)  # 只拖出不接收
    ...
    def _emit_flatten(self, slot, top_name):
        # 通过主窗口处理，这里直接触发 flatten_requested 信号
        # 我们借道 curve_selected 信号，用 '__flatten__:topname' 格式传递
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("WellCorrelator v6.5 · StarSteer Style")
    ...
    def _build_top_bar(self):
        ...
        inner_lay = QHBoxLayout(self._top_bar_content)
        ...
        # ── 参考井 A 标签 ──
        inner_lay.addWidget(lbl("A", "wellTag_A"))
        inner_lay.addWidget(self._make_top_divider())

        # Y 轴深度范围
        inner_lay.addWidget(lbl("深度:"))
        ...
        inner_lay.addWidget(self._tb_le_dmin_a)
        inner_lay.addWidget(QLabel("–"))
        inner_lay.addWidget(self._tb_le_dmax_a)
        ...
        inner_lay.addWidget(btn_apply_d_a)

        inner_lay.addWidget(self._make_top_divider())

        # X 轴曲线范围 (A)
        inner_lay.addWidget(lbl("曲线范围:"))
        ...
        inner_lay.addWidget(self._tb_le_vmin_a)
        inner_lay.addWidget(QLabel("–"))
        inner_lay.addWidget(self._tb_le_vmax_a)
        ...
        inner_lay.addWidget(btn_apply_v_a)

        inner_lay.addWidget(self._make_top_divider())

        # 充填模式 (A)
        inner_lay.addWidget(lbl("充填:"))
        ...
        inner_lay.addWidget(self._tb_cmb_fill_a)
        ...
        inner_lay.addWidget(self._tb_btn_ghost_a)

        # Batch to Top A
        btn_batch_a = QPushButton("批量分层 A")
        btn_batch_a.clicked.connect(lambda: self._batch_add_tops(slot='A'))
        inner_lay.addWidget(btn_batch_a)

        inner_lay.addWidget(self._make_top_divider())

        # ── 目标井 B 标签 ──
        inner_lay.addWidget(lbl("B", "wellTag_B"))
        inner_lay.addWidget(self._make_top_divider())
        ...
        inner_lay.addWidget(self._tb_le_vmin_b)
        inner_lay.addWidget(QLabel("–"))
        inner_lay.addWidget(self._tb_le_vmax_b)
        ...
        inner_lay.addWidget(btn_apply_v_b)
        ...
        self._tb_cmb_fill_b.currentTextChanged.connect(
            lambda t: self.panel_b.set_fill_mode(
                {'无': 'none', '左充填': 'left', '右充填': 'right', '双向': 'both'}.get(t, 'both')))
        inner_lay.addWidget(self._tb_cmb_fill_b)
